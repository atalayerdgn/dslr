    ...
    def max_data(self, column_name):
        ...
        for x in numerical_values:
            if x > max_val:
                max_val = x
        return max_val
    def Skewness_data(self, column_name):
        numerical_values = [x for x in self.data[column_name] if isinstance(x, (int, float))]
        if not numerical_values:
            return None
        mean_val = sum(numerical_values) / len(numerical_values)
        std_val = self.std_data(column_name)
        return sum((x - mean_val) ** 3 for x in numerical_values) / (len(numerical_values) * std_val ** 3)
    def Kurtosis_data(self, column_name):
        numerical_values = [x for x in self.data[column_name] if isinstance(x, (int, float))]
        if not numerical_values:
            return None
        mean_val = sum(numerical_values) / len(numerical_values)
        std_val = self.std_data(column_name)
        return sum((x - mean_val) ** 4 for x in numerical_values) / (len(numerical_values) * std_val ** 4)
    ...
    def describe_data(self):
        for column_name in self.columns:
            ...
            if numerical_values:
                ...
                print(f"Skewness: {self.Skewness_data(column_name)}")
                print(f"Kurtosis: {self.Kurtosis_data(column_name)}")
                print("-" * 30)
